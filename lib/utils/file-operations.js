 ...
 *
 * Path Replacement:
 * - Replaces docs/ and plans/ with custom paths if configured
 */
...
import { replacePathsInContent, DEFAULTS } from './path-config.js';
 ...
 * @param {object} options - { dryRun, force, paths }
...
export async function mergeClaudeFolder(templateClaudeDir, targetClaudeDir, options = {}) {
  const copied = [];
  const conflicts = [];
  const paths = options.paths || { docs: DEFAULTS.docs, plans: DEFAULTS.plans };

  /**
   * Copy file or folder with path replacement for .md files
   */
  async function copyWithReplacement(src, dest) {
    const stat = await fs.stat(src);
    if (stat.isDirectory()) {
      await fs.ensureDir(dest);
      const items = await fs.readdir(src);
      for (const item of items) {
        await copyWithReplacement(join(src, item), join(dest, item));
      }
    } else if (src.endsWith('.md')) {
      const content = await fs.readFile(src, 'utf-8');
      const replaced = replacePathsInContent(content, paths);
    ...
    for (const item of templateItems) {
      ...
      if (await fs.pathExists(targetItemPath)) {
        if (options.force) {
          ...
          if (!options.dryRun) {
            await copyWithReplacement(templateItemPath, targetItemPath);
          }
        } else {
 ...
 * @param {object} options - { dryRun, force, paths: { docs, plans } }
 * @returns {Promise<{copied: string[], conflicts: {path: string, action: string}[]}>}
 */
export async function copyTemplate(version, targetDir, options = {}) {
  ...
  /**
   * Copy file with path replacement
   * @param {string} src - Source file path
   * @param {string} dest - Destination file path
   */
  async function copyWithPathReplacement(src, dest) {
    const content = await fs.readFile(src, 'utf-8');
    const replaced = replacePathsInContent(content, paths);
    await fs.writeFile(dest, replaced, 'utf-8');
  }
  ...
  for (const file of TEMPLATE_FILES) {
    ...
    // Use path replacement for .md files
    if (file.endsWith('.md')) {
      await copyWithPathReplacement(src, dest);
    } else {
      await fs.copy(src, dest, { overwrite: true });
    }
    copied.push(file);
  ...
  // Handle .claude folder with smart merge (pass paths for replacement)
  const templateClaudeDir = join(templateDir, '.claude');
  const targetClaudeDir = join(targetDir, '.claude');

  if (await fs.pathExists(templateClaudeDir)) {
    const mergeResult = await mergeClaudeFolder(templateClaudeDir, targetClaudeDir, { ...options, paths });
