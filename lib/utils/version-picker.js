 ...
 * @returns {Promise<{version: string, versionInfo: object}>} Selected version and its info
 */
export async function pickVersion() {
  ...
  if (process.env.IPA_CK_VERSION) {
    ...
    return { version: testVersion, versionInfo: found };
  }

  const choices = versions.map(v => {
    let label = `v${v.version}`;
    if (v.version === latest) label += ' (latest)';
    if (v.recommended) label += ' ★';
    label += ` - ${v.notes}`;

    // Add warning indicator for versions that require special features
    if (v.warning) {
      label += ' ⚠️';
    }
